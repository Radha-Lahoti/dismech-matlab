function [dist, contact_type, tri_pair_updated, tri_pair_nodes_updated, ratios] = modified_triangleContactType(q, tri_pair_nodes, scale)
...
%   tri_pair_updated: rearranged triangle vertices
    ...
    for i = 1:3
        for j = 1:3
            a = V2(:,j);
            b = V2(:,mod(j,3)+1);
            [d, cp, ratio] = pointToSegmentDist(V1(:,i), a, b);
            if d < min_dist
                min_dist = d;
    ...
    for i = 1:3
        for j = 1:3
            a = V1(:,j);
            b = V1(:,mod(j,3)+1);
            [d, cp, ratio] = pointToSegmentDist(V2(:,i), a, b);
            if d < min_dist
                min_dist = d;
                min_dist2 = d; % to debug
                contact_type = "PointToEdge";
    ...
    for i = 1:3
        ...
        for j = 1:3
            a2 = V2(:,j);
            b2 = V2(:,mod(j,3)+1);
            [d, p1, p2, ratio1, ratio2] = segmentToSegmentDist(a1, b1, a2, b2);
            if d < min_dist
                min_dist = d;
                ...
                ratios = zeros(2,3);
                ratios(1,i) = ratio1(1);
                ratios(1,mod(i,3)+1) = ratio1(2);
                ratios(2,j) = ratio2(1);
                ratios(2,mod(j,3)+1) = ratio2(2);
            end
        end
    end
    ...
    for i = 1:3
        [d, is_inside, bary] = pointToTriangleDist(V1(:,i), V2);
        ...
        [d, is_inside, bary] = pointToTriangleDist(V2(:,i), V1);
        if is_inside && d < min_dist
            min_dist = d;
            min_dist4 = d; % to debug
            contact_type = "PointToFace";
    ...
    for i = 1:3
        a = V1(:,i);
        b = V1(:,mod(i,3)+1);
        [intersects, pt, tri_bary, seg_bary] = segmentTriangleIntersection(a, b, V2);
        if intersects
            if norm(pt - a) > 1e-8 && norm(pt - b) > 1e-8
                ...
                % set ratios
                ratios = zeros(2,3);
                ratios(1,i) = seg_bary(1);
                ratios(1,mod(i,3)+1) = seg_bary(2);
                ratios(2,:) = tri_bary;
        ...
        a = V2(:,i);
        b = V2(:,mod(i,3)+1);
        [intersects, pt, tri_bary, seg_bary] = segmentTriangleIntersection(a, b, V1);
        if intersects
            if norm(pt - a) > 1e-8 && norm(pt - b) > 1e-8
                min_dist = 0;
                min_dist5 = 0; % to debug
                ...
                % set ratios
                ratios = zeros(2,3);
                ratios(2,i) = seg_bary(1);
                ratios(2,mod(i,3)+1) = seg_bary(2);
                ratios(1,:) = tri_bary;
...
min_distances = [min_dist1, min_dist2, min_dist3, min_dist4, min_dist5];
    ...
    elseif contact_type == "PointToEdge"
        ...
        if ismember(point', V1', 'rows')
            ...
            ratios(1,:) = circshift(ratios(1,:), [0, 1 - idx_point]);
            ...
            ratios(2,:) = circshift(ratios(2,:), [0, 1 - idx_edge_start]);
        else
            ...
            ratios(2,:) = circshift(ratios(2,:), [0, 1 - idx_point]);
            ...
            ratios(1,:) = circshift(ratios(1,:), [0, 1 - idx_edge_start]);
    ...
    if(need_to_switch)
        ...
        ratios = flipud(ratios); % since triangle 1 and 2 were swapped
    else
