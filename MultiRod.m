classdef MultiRod
    ...
    methods
        function obj = MultiRod(geom, material, twist_angles, Nodes, Edges, rod_edges, shell_edges, rod_shell_joint_edges, rod_shell_joint_total_edges, face_nodes, sign_faces, face_edges, face_shell_edges, sim_params, environment)
            ...
            if(ismember("gravity",environment.ext_force_list))
            ...
            obj.tri_combos = obj.construct_possible_triangle_combos(face_nodes);
    ...
    methods (Static)
        ...
        function [tri_combos, tri_combos_idx] = construct_possible_triangle_combos(triangles)
            % Construct list of all possible edge combinations without duplicates (excluding adjacent edges)
            % Inputs:
            % triangles:- n_triangles*2 array of edge node indices
            % Outputs:
            % tri_combos:- no. of possible triangles_combos for collision*4 (node indices xi, xi+1, xj, xj+1)
            % tri_combos_idx:- no. of possible triangles_combos for collision*2 (triangle indices ti, tj)
            % ___________________________________________________________________________________________

            no_of_triangles = size(triangles, 1);

            tri_combos_idx = [0, 0]; % jugaad for using ismember
            for i=1:no_of_triangles
                for j=i+1:no_of_triangles
                    temp_combo = [i , j];
                    % check if edge is itself or adjacent
                    if(any(ismember(triangles(i,:), triangles(j,:))))
                        % not valid combination
                    elseif (ismember(temp_combo, tri_combos_idx, "rows") || ismember([j,i], tri_combos_idx,"rows"))
                        % already counted combination
                    else
                        tri_combos_idx = [tri_combos_idx; temp_combo];
                    end
                end
            end
            tri_combos_idx = tri_combos_idx(2:end,:); % remove the jugaad for using ismember
            tri_combos = zeros(size(tri_combos_idx,1),6);

            for k = 1:size(tri_combos_idx,1)
                tri_combos(k,:) = [triangles(tri_combos_idx(k,1),:), triangles(tri_combos_idx(k,2),:)];
            end
